...
async function getRecipes() {
  ...
  const localList = localStorage.getItem('recipeList');
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (const url of RECIPE_URLS)
    {
      ...
      try
      {
        ...
        const recipeFile = await response.json();

        // A8. TODO - Add the new recipe to the recipes array

        recipeList.push(recipeFile);
        ...
        {
          saveRecipesToStorage(recipeList); // given function
          resolve(recipeList);
        }
      } 
      catch (error)
